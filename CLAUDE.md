...
- `questlit/ui/data.py` — `@st.cache_data` loaders (`load_accounts`, `load_positions`, `load_balances`, `load_orders`, `load_activities`, `load_candles`). TTL is 1 day for accounts, 60s for the rest. Pages must import from this module so caches are shared app-wide.
