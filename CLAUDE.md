...
- `pages/candles.py` — minimal OHLCV chart for a single ticker via Plotly (`make_subplots` with a candlestick row + volume bar row, rangeslider hidden). Inputs: symbol text box, interval selectbox (curated subset of `CANDLE_INTERVALS`), a "Range" text box (`Nd` / `Nw` / `Nm` / `Ny`, default `3m`) and an End date — Start is computed as `End - Range` and rendered in a disabled `date_input` for display only. Chart title shows the symbol's `description` and `listingExchange` from `load_symbol_info`. Backed by `load_candles` and `load_symbol_info` in `questlit/ui/data.py`.
- `pages/closed_trades.py` — past/closed-trade page. Reuses `account_dates_sidebar` (sidebar), `summarize_closed_trades` (per-symbol realized PnL) and `show_activities` (deep-dive) from `questlit/ui/`. Computes `closed_sym` (symbols in activities but not in current positions), splits the fully-closed round-trips into **Winners** / **Losers** tabs and lists symbols that don't reconcile in the window under **Incomplete**. A sidebar selectbox (annotated with each symbol's PnL) drives the `show_activities` Overview/Trades/Dividends deep-dive.
- `questlit/ui/` — Streamlit-only helpers: `auth.py` (seed-token gate), `data.py` (cached loaders), `formatting.py` (dataframe column-config helpers), `controls.py` (shared sidebar + relocated `_parse_range`/`get_st_app_url`), `trades.py` (trade-analysis helpers reused across pages).
...
- `questlit/ui/data.py` — `@st.cache_data` loaders (`load_accounts`, `load_positions`, `load_balances`, `load_orders`, `load_activities`, `load_candles`, `load_symbol_info`). TTL is 1 day for accounts and symbol metadata, 60s for the rest. Pages must import from this module so caches are shared app-wide.
- `questlit/ui/formatting.py` — `currency_column_config(df)` builds a `st.column_config.NumberColumn(format="$%.2f")` mapping for every numeric column in `df` (`exclude_cols` skips e.g. ratio/quantity columns); `position_df_styler(df)` colors columns ending in `Pnl` / `_yield` / `_return` (and `stopProfitPct`) green/red. Reuse for any dollar-denominated table.
- `questlit/ui/controls.py` — page-independent helpers and the shared sidebar, kept here so the `ui` layer never imports from a `pages/*` module. `account_dates_sidebar()` renders the Account / Balance / Dates sidebar and returns `(target_acc, start_ts, end_ts)` (used by `positions.py` and `closed_trades.py`); `_parse_range(s, end)` and `get_st_app_url(qp)` were relocated here from `pages/candles.py` (which now imports them from `controls`).
- `questlit/ui/trades.py` — trade-analysis helpers reused across pages (kept out of `pages/*` so importing them never triggers a page's top-level `main()`): `show_activities(symbol, df_pos, df_activities, st_container)` renders the per-symbol Overview/Trades/Dividends deep-dive (handles both open positions and closed trades); `closed_trade_pnl(symbol, df_activities)` returns a realized-PnL dict (`realizedPnl = proceeds − cost + dividends`, plus `is_complete` when buy/sell quantities reconcile); `summarize_closed_trades(closed_syms, df_activities)` builds the one-row-per-symbol DataFrame the closed-trades page splits into winners/losers/incomplete.
